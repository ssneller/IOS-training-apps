...
+ (NSDictionary *)requestUserInfo:(NSString *)username   /// method name and username is parameter passed
{
    ...
    NSURLRequest * request = [NSURLRequest requestWithURL:url]; /// set vaiable for request
...
+ (NSString *)usersArchivePath
{
    ...
    ///returns arra of dircetories
    NSArray * documentDirectories = NSSearchPathForDirectoriesInDomains(NSDocumentationDirectory, NSUserDomainMask, YES);
    
    NSString * documentDirectory = documentDirectories[0];
    
    return  [documentDirectory stringByAppendingPathComponent:A"users.data"];
